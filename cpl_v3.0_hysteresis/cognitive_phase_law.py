...
def compute_observer_entropy(z: np.ndarray, n_clusters: int = 10) -> Tuple[float, float]:
    ...
    Returns:
    entropy_value : float
        Shannon entropy (in nats, base e) of the cluster distribution.
    stability : float
    ...
    # 3. ENTROPY CALCULATION (in nats, using natural logarithm)
    entropy_value_nats = scipy_entropy(probabilities, base=np.e)
    
    # 4. STABILITY CALCULATION
    stability = np.max(probabilities)
    
    # Return entropy in nats for full consistency with article and cpl_core.py
    return entropy_value_nats, stability
...
def classify_cognitive_phase(H_obs: float, S: float, dH_dt: float,
    ...
    H_c : float     # Critical entropy threshold (in nats, default ln(3))
...
if __name__ == "__main__":
    ...
    # Create 2 clusters for "Coherent" phase with one dominant cluster (>=70%)
    np.random.seed(42)
    # Dominant cluster: 70 samples tightly concentrated -> S > 0.7
    cluster_1 = np.random.normal(loc=[0, 0], scale=0.1, size=(70, 2))
    # Secondary cluster: 30 samples more dispersed
    cluster_2 = np.random.normal(loc=[0.5, 0.5], scale=0.3, size=(30, 2))
    z_coherent = np.vstack([cluster_1, cluster_2])
    
    # Create uniform distribution for "Fragmented" phase (high entropy)
    ...
    print(f"   Entropy (H_obs): {H_obs_coherent:.3f} nats")
    ...
    print(f"   Entropy (H_obs): {H_obs_frag:.3f} nats")
    ...
    print(f"   Entropy (H_obs): {H_obs_current:.3f} nats")
    print(f"   Stability (S): {S_current:.3f}")
    print(f"   Entropy change rate (dH_dt): {dH_dt_reorg:.3f}")
    print(f"   Predicted phase: {phase}")
    
    # Print critical thresholds for reference
    print(f"\n--- CRITICAL THRESHOLDS ---")
    print(f"   H_c (critical entropy): {np.log(3):.3f} nats")
    print(f"   S_c (critical stability): {0.7:.1f}")
    print(f"   γ (critical rate): {0.1:.1f}")